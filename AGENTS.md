...
│   ├── main.go             Wails 入口
│   ├── app.go              暴露给前端的 bindings（NewSession / GetEndpoint / 等）
...
4. **协议向后兼容**：`internal/proto.Version = 1`。新增帧类型只能用未占用的 `Type` 字节；不能改现有帧的 payload 结构。任何 wire 变更都要更新 `doc/spec/protocol.md`。
...
- `doc/spec/architecture.md` — 整体架构、组件职责、数据流、phase 路线
- `doc/spec/protocol.md` — wire 协议完整规范（所有帧类型、重连续传语义）
- `doc/spec/conventions.md` — Go/TS 代码约定、commit 风格、测试组织
