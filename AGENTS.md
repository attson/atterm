...
20. **OPAQUE 套件 = P-256-SHA256 + Scrypt**：Go 端 `internal/relay/opaque_server.go` 的 `opaqueSuiteTag = "p256-scrypt-v1"` 和 TS 端 `@cloudflare/opaque-ts` 的 `OpaqueID.OPAQUE_P256` 绑死。bytemare 默认是 ristretto255-SHA512，但 Cloudflare TS 不支持，所以**不要**给"安全升级"为别的曲线——会让浏览器/桌面之间的握手直接失败。`SERVER_IDENTITY = "atterm-relay"` 也是两端硬编码。**KSF 拉伸长度必须两端一致**：桌面 `bytemare/opaque` 把密码 Harden 到 OPRF element 长度（P-256 = **33 字节**），所以 web 端**不能**用 `@cloudflare/opaque-ts` 默认的 `ScryptMemHardFn`（dkLen=32），必须用 `web/src/shared/lib/opaque.ts` 里 dkLen=**33** 的 `opaqueStretch`——否则 `randomized_pwd` 不一致，一端注册的账号在另一端登录会报 `invalid credentials`。两端用同一条 golden vector 锁死（`internal/e2eeclient` 的 `TestOpaqueKSFGoldenVector` 与 `web/tests/unit/opaque.test.ts`）。详见 [docs/spec/auth.md](./docs/spec/auth.md) §OPAQUE。
