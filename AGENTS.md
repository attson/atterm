...
阅读这份文件以快速上手；详细规范见 [`docs/spec/`](./docs/spec/)。历史 implementation plan 一句话归档见 [`docs/plans-history.md`](./docs/plans-history.md);为什么这样设计的架构决策见 [`docs/superpowers/specs/`](./docs/superpowers/specs/)(常被代码注释直接引用,不要动)。**当前有哪些 feature、什么时候上的**——一律 `git log --oneline -30` / `gh pr list --state merged --limit 30`,本文档不维护流水账。
...
22. **AAD 鉴别字节是 cross-type replay 的唯一防线**：每个 sealed 信封的 AAD = `uuid(16B) || frame_type(1B)`。当前分配：`IN=0x02` / `OUT=0x03` / `META=0x05` / `LIST_RESP/SessionInfo=0x12` / `PASTE_IMAGE=0x33` / `COMMAND_EVENT=0x35` / `PASTE_FILE=0x37` / `FS_REQUEST=0x38` / `FS_RESPONSE=0x39` / `FS_EVENT=0x3a` / `SERVICE_OPEN=0x3d` / `ssh_hosts_encrypted sync(合成,不上 wire)=0xF0` / `profiles_encrypted sync(合成,不上 wire)=0xF1`。加新 sealed 帧类型时**必须**给新 frame_type 一个唯一字节，并在 [docs/spec/protocol.md](./docs/spec/protocol.md) §E2EE 信封 加一行，否则别处的信封能被替换重放。这份分配表是 `internal/e2eecrypto/aadtags.go`（`AADTags` 注册表）的复制品——`aadtags_test.go` 只把注册表钉在 `protocol.md` 上，不会检测这里；改一处务必手动同步另一处。
...
环境变量参考见 skill **`atterm-map`**（`.claude/skills/atterm-map/SKILL.md`）§环境变量。

## 何时改哪里

「某个场景要动哪些文件」的完整对照表已迁到 skill **`atterm-map`**（`.claude/skills/atterm-map/SKILL.md`），按需加载。环境变量参考同在该文件。

## 风格摘要

- Go：`gofmt`，包注释 + 公共 API 注释（说"为什么"）；errors `%w` wrap；日志走 `internal/logging`（`Debug/Info/Warn/Error(tag, ...)`，desktop package main 有 `logDebug/...` 短别名），不引入 logrus/zap；裸 `log.Printf` 由 `internal/logging` 的 AST 测试拦截。级别口径与 tag 词表见 [conventions.md](./docs/spec/conventions.md) §日志
