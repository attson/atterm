import type { Messages } from "./en";

export const zhCN = {
  common: {
    language: "语言",
    system: "跟随系统",
    english: "英语",
    simplifiedChinese: "简体中文",
    save: "保存",
    cancel: "取消",
    close: "关闭",
    refresh: "刷新",
    copy: "复制",
    paste: "粘贴",
    send: "发送",
    disconnect: "断开连接",
    connect: "连接",
    settings: "设置",
    loading: "加载中...",
    disabled: "已禁用",
    discard: "放弃",
    stay: "留在此处",
    delete: "删除",
    retry: "重试",
    unknown: "(未知)",
    countSessions: "{count} 个会话",
    countSessionsOne: "1 个会话",
    minimize: "最小化",
    maximize: "最大化",
    restore: "还原",
    yes: "是",
    no: "否",
  },
  app: {
    authInvalidToken: "API token 无效或已撤销。请在网页设置中生成新的 token。",
    authUserDisabled: "账号已禁用。请联系 relay 管理员。",
    sessionIdOwnerMismatch: "会话 ID 冲突。请重启桌面应用。",
    forbidden: "无权连接到此 relay。",
    openSettings: "打开设置",
    dismiss: "关闭提示",
    showPanel: "显示面板",
    hidePanel: "隐藏面板",
    startingFirstSession: "正在启动第一个会话...",
    waitingForSessionSelection: "等待会话选择...",
    noShellsFound: "此机器上找不到 shell",
    wailsBindingsUnavailable: "Wails 绑定不可用",
    wailsBindingsNotReady: "Wails 绑定尚未就绪",
    sessionAlreadyInTab: "该会话已在当前标签页",
    paneFull: "面板已满 - 请先关闭一个",
    splitFailed: "分屏失败：{message}",
    noActiveSession: "没有活动会话",
    noEndpoint: "没有 endpoint",
    pluginFailedToLoad: "插件“{title}”加载失败",
    startupFailureTitle: "AT Term 启动失败",
    startupFailureIntro: "桌面后端遇到了致命启动错误。终端会话尚未启动。",
    startupFailureLogPath: "日志文件",
    startupFailureCopy: "复制诊断信息",
    startupFailureCopied: "启动诊断信息已复制",
  },
  titlebar: {
    e2eeOffChip: "明文",
    e2eeOffTitle: "已禁用端到端加密：本机 agent 现在以明文向 relay 发送终端输出、会话标题 / cwd / 当前命令、命令完成通知。在 Settings → Remote relay 关闭即可恢复加密。",
  },
  terminal: {
    route: {
      relay: "Relay",
      connectingDirect: "正在建立直连",
      direct: "直连",
      current: "当前路径：{route}",
      iceState: "ICE 状态：{state}",
      candidateType: "候选类型：{type}",
      setupTime: "建立耗时：{ms} 毫秒",
      fallbackReason: "回退原因：{reason}",
      fallback: {
        accountKeyUnavailable: "E2EE 密钥未解锁",
        signalEndpointUnavailable: "直连信令不可用",
        webrtcUnavailable: "当前运行环境不支持 WebRTC",
        timeout: "直连建立超时",
        signalingRejected: "信令被拒绝",
        hostUnavailable: "主机直连端点不可用",
        iceFailed: "ICE 连通失败",
        authenticationFailed: "对端认证失败",
        backpressure: "直连通道拥塞",
        protocolError: "直连协议错误",
        directDisconnected: "直连通道已断开",
        preferenceDisabled: "直连已关闭",
        transportError: "直连传输失败",
      },
    },
    connecting: "连接中...",
    reconnecting: "重新连接中...",
    ended: "会话已结束",
    sessionEndedBanner: "[AT Term] 会话已结束（退出码 {exitCode}）",
    connectionError: "连接错误",
    remoteHasTakenControl: "远端已接管控制",
    byHost: "来自 {host}",
    pressSpaceToTakeBack: "按空格取回控制",
    takeControl: "接管控制",
    takeControlQueued: "Relay 正在重连，接管请求已排队",
    clearBuffer: "清空缓冲区",
    search: {
      placeholder: "查找",
      next: "下一个匹配",
      prev: "上一个匹配",
      close: "关闭搜索",
      noResults: "无匹配",
    },
    sendSelection: "发送",
    pickImage: "图片",
    pickFile: "文件",
    emptyPaneHint: "[空面板 - 按 Cmd+N / Ctrl+N 填充]",
    remote: "远端",
    remotePrefix: "[远端] ",
    shellFallback: "shell",
    emptyTab: "(空)",
    tabDisconnectedSuffix: "(已断开)",
    closePaneTitle: "关闭面板 (Cmd+W / Ctrl+W)",
    dragPaneOut: "拖到标签条可独立成标签页",
    detachToTab: "独立标签页",
    alreadyOwnTab: "该会话已是独立标签页",
    remoteViewerWatching: "{count} 个远端查看者正在观看",
    relaySettings: "relay 设置",
    adminPanel: "管理面板",
    newTab: "新建标签页",
    starting: "启动中...",
    newTabProfileLabel: "新建标签页 / 分屏用的配置档",
    useDefaultProfile: "默认配置档",
    layoutVertical: "左右分屏",
    layoutHorizontal: "上下分屏",
    layoutGrid2x2: "2x2 网格分屏",
    sessionStatusOne: "{count} 个会话",
    sessionStatusMany: "{count} 个会话",
    uplinkOn: "uplink 已开启",
    imagePasteRequiresFull: "粘贴图片需要 full 远端权限",
    pasteSessionNotWritable: "当前会话暂时不可写入",
    clipboardEmpty: "剪贴板没有可粘贴的文本或图片",
    clipboardImageTooLarge: "剪贴板图片过大",
    clipboardImageToolsMissing: "请安装 xclip、wl-paste 或 xsel 以粘贴图片",
    readOnlySession: "会话为只读",
    websocketNotOpen: "websocket 未打开",
    imageTooLarge: "图片过大：{size} 字节，超过 {limit}",
    copyFailed: "复制失败",
    pasteFailed: "粘贴失败",
    driverNow: "你现在是控制者",
    viewerNow: "你现在是查看者",
    preview: {
      open: "预览 Web 服务",
      opening: "正在打开…",
      portPrompt: "输入远端桌面上 Web 服务的 loopback 端口（例如 3000）",
      targetHost: "远程服务 loopback 地址",
      additionalPort: "附加服务端口",
      pathPrefix: "路径前缀",
      addMapping: "添加服务映射",
      removeMapping: "移除服务映射",
      invalidPathPrefix: "附加服务映射必须使用不同的路径前缀（例如 /api、/ws）",
      desktopOnly: "远程预览仅在桌面客户端可用",
      requiresConnection: "请等待远程会话连接后再打开预览",
      requiresDriver: "请先接管远程会话，再打开预览",
      requiresFullPermission: "远程预览需要主机授予完整权限",
      invalidPort: "端口必须是 1–65535 的整数",
      noLongerAllowed: "打开过程中 Preview 权限已失效",
      openFailed: "无法打开远程预览：{error}",
      terminal: "终端",
      previewPort: "预览 :{port}",
      switch: "切换预览",
      closeThis: "关闭此预览",
      newPreview: "新开预览…",
      reconnecting: "正在重连…",
      frameTitle: "远程 Web 预览 :{port}",
      openInBrowser: "在系统浏览器打开",
      copyURL: "复制预览地址",
      urlCopied: "预览地址已复制",
      browserOpenFailed: "无法打开系统浏览器",
      copyFailed: "复制预览地址失败",
    },
    bellNotification: "{session} 中有响铃",
    commandFinishedNotification: "命令已完成 - 退出码 {exitCode} - {elapsed} - {session}",
    loadingHistoryBytes: "正在加载历史 - {bytes}",
    loadingHistoryProgress: "正在加载历史 {pct}% - {bytes} / {total}",
    contextMenu: {
      openLink: "打开链接",
      copyLink: "复制链接",
    },
    resize: "适配终端尺寸",
    link: {
      openFailed: "无法打开链接",
      openFailedNoHome: "无法解析 ~（拿不到 home 目录）",
    },
  },
  sessions: {
    local: "本地",
    remote: "远端",
    remoteSessions: "远端会话",
    hostIdTitle: "host_id {hostId}",
    sessionTitle: "会话 {sessionId}",
    cancelEsc: "取消 (esc)",
    endLocalSessionOne: "结束 {count} 个本地 shell 会话",
    endLocalSessionMany: "结束 {count} 个本地 shell 会话",
    detachRemoteSessionOne: "从 {count} 个远端会话分离",
    detachRemoteSessionMany: "从 {count} 个远端会话分离",
    runningProcessesTerminated: "(运行中的进程将被终止)",
    remotePtyKeepsRunning: "(远端 PTY 会继续在其主机上运行)",
    saveWorkFirst: "请先保存你的工作。",
    quitTitle: "退出 AT Term",
    closingWill: "关闭 AT Term 将会：",
    quit: "退出",
    closeSessionTitle: "关闭会话？",
    closeSessionBody: "确认关闭「{title}」？",
    closeSessionManyBody: "确认关闭 {count} 个会话？",
    closeAiSessionWarning: "这是 AI 会话，关闭前请确认任务已经结束或已保存需要的上下文。",
    closeRunningSessionWarning: "这是运行中的会话，关闭可能中断正在执行的命令。",
    closeLocalTerminateHint: "本地会话会被结束，运行中的进程将被终止。",
    closeRemoteDetachHint: "远端会话会从当前窗口分离，远端 PTY 会继续运行。",
    closeSessionConfirm: "确认关闭",
    unknownHost: "未知主机",
    thisMachine: "本机",
  },
  ssh: {
    tabHosts: "主机",
    tabKeys: "密钥",
    filterPlaceholder: "筛选主机…",
    newHost: "新建主机",
    newKey: "新建密钥",
    edit: "编辑",
    subtitleAuthKey: "密钥",
    subtitleAuthPassword: "密码",
    importedHosts: "已导入 {count} 台主机",
    hosts: {
      emptyTitle: "还没有保存的主机",
      emptySub: "添加一台主机后就能连接它，并在任意设备上接管这个会话。",
      clearTags: "清除",
      noMatchTitle: "没有匹配的主机",
      noMatchSub: "放宽筛选条件，或清除已选中的标签。",
      menuConnect: "连接",
      menuEdit: "编辑主机信息",
      menuDuplicate: "复制一份",
      menuCopySsh: "复制 SSH 命令",
      menuRemove: "移除",
    },
    keys: {
      emptyTitle: "还没有密钥",
      emptySub: "先在这里添加私钥，再在主机里引用它。",
      typeLabel: "类型 {type}",
      genericSubtitle: "SSH 密钥",
    },
    proxy: {
      jumpBadge: "经 {target}",
      commandBadge: "ProxyCommand",
      jumpReason: "经跳板机 {target} 连接。链路上每一跳都必须是 atterm 里已保存的主机——atterm 用那台主机自己的凭据登录它，绝不会把这台主机的凭据发过去。",
      commandReason: "配置了 ProxyCommand（{command}）：atterm 不会执行这条命令，所以这台主机不能直接连。",
    },
    hostKey: {
      title: "未知的主机密钥",
      direct: "未知的主机密钥。信任之前请先核对这个指纹：",
      jumpHop: "这个指纹属于 {name}，是通往 {target} 途中的第 {hop} 跳，不是 {target} 本身。请对着 {name} 核对之后再信任：",
      targetHop: "这个指纹属于 {target}，也就是你要连的那台主机（在链路上是第 {hop} 跳）。核对之后再信任：",
      chainNote: "链路上每台机器都会单独询问一次；接受这一台不代表接受其它任何一台。",
      accept: "接受并连接",
      reject: "取消",
    },
    hostDrawer: {
      titleNew: "新建主机",
      titleEdit: "编辑主机",
      address: "地址",
      addressPlaceholder: "IP 或主机名",
      label: "别名",
      labelPlaceholder: "可不填",
      port: "端口",
      tags: "标签",
      tagsPlaceholder: "用逗号分隔",
      removeTag: "移除 {tag}",
      username: "用户名",
      usernamePlaceholder: "user",
      authPassword: "密码",
      authKey: "密钥",
      identityFileLabel: "来自 ~/.ssh/config",
      identityFileHint: "先到「密钥」里导入这个私钥，再回来在下面选中它。",
      password: "密码",
      keepBlank: "（留空表示不修改）",
      keyPicker: "密钥",
      keyPickerAria: "SSH 密钥",
      noKeys: "还没有密钥。",
      addKey: "添加密钥",
      submitNew: "添加主机",
    },
    keyDrawer: {
      titleNew: "新建密钥",
      titleEdit: "编辑密钥",
      name: "名称",
      namePlaceholder: "例如 aws",
      pem: "私钥（PEM）",
      keepBlank: "（留空表示不修改）",
      passphrase: "口令",
      encryptedHint: "这个私钥是加密的，请在上面填写它的口令。",
      dropzone: "把私钥文件拖到这里导入",
      importFromFile: "从密钥文件导入",
      submitNew: "添加密钥",
      reveal: "查看已保存的私钥",
      hide: "隐藏",
      copy: "复制私钥",
    },
    startup: {
      sectionTitle: "启动命令",
      sectionHint: "给那些登录后先给一个菜单、而不是直接给 shell 的堡垒机用。每行会依次输入并回车，一行对应一个提示符——JumpServer 就是先在 Opt> 选资产，再在 ID> 选系统用户。",
      placeholder: "一行一个回答",
      delay: "步骤间隔（毫秒）",
      steps: "登录后会依次发送 {count} 步。",
    },
    forwards: {
      tab: "隧道",
      sectionTitle: "端口转发",
      sectionHint: "规则只是配置，要到「隧道」里手动启动才会生效。",
      add: "添加规则",
      remove: "删除这条规则",
      noRules: "还没有转发规则。",
      kindLocal: "本地",
      kindRemote: "远程",
      kindDynamic: "动态",
      kindLocalHint: "本机监听，每条连接由 SSH 主机去连目标（ssh -L）。",
      kindRemoteHint: "SSH 主机监听，它收到的每条连接由本机去连目标（ssh -R）。",
      kindDynamicHint: "本机以 SOCKS5 代理监听，每条连接自带目标地址，由 SSH 主机解析（ssh -D）。",
      bindAddr: "监听地址",
      bindPort: "端口",
      targetHost: "目标主机",
      targetPort: "目标端口",
      label: "备注",
      labelPlaceholder: "可不填",
      warnLocal:
        "监听 {addr} 而不是 127.0.0.1，意味着同网段的任何人都能经由你这台机器访问这个被转发的服务，不需要任何 SSH 凭据。这条规则会同步到你登录的每台设备，在那些设备上也这样监听。",
      warnRemote:
        "监听 {addr} 而不是 127.0.0.1，意味着 SSH 主机会接受同网段任何人的连接（不需要任何 SSH 凭据），并经由你这台机器转给下面的目标。远端 sshd 默认的 GatewayPorts 通常会拒绝这种绑定。这条规则会同步到你登录的每台设备。",
      warnDynamic:
        "监听 {addr} 而不是 127.0.0.1，会让它变成一个无认证的 SOCKS5 开放代理：同网段的任何人都能访问这台 SSH 主机能访问的一切，目标由调用方自己指定，用的是你的 SSH 凭据，在对端日志里显示成你。这条规则会同步到你登录的每台设备，在那些设备上也这样监听。",
      emptyTitle: "还没有转发规则",
      emptySub: "可以直接在这里新建，也可以在某台主机的「端口转发」里添加。",
      new: "新建隧道",
      newNeedsHost: "先保存一台 SSH 主机——隧道要跑在主机的连接上。",
      drawerTitleNew: "新建隧道",
      drawerTitleEdit: "编辑隧道",
      hostPicker: "SSH 主机",
      hostPickerAria: "这条隧道所走的 SSH 主机",
      hostPickerHint: "隧道走这台主机的连接——目标在跳板机后面时，这里选的就是跳板机。它会保存成这台主机的一条转发规则。",
      saveAndStart: "保存并启动",
      start: "启动",
      stop: "停止",
      dismiss: "清除",
      stateIdle: "未启动",
      stateRunning: "运行中",
      stateStopped: "已停止",
      conns: "已接受 {count} 条",
      dynamicTarget: "SOCKS5（目标由每条连接指定）",
      proxyBlocked: "这台主机不能启动隧道。",
      refresh: "刷新",
    },
    configImport: {
      open: "从 ~/.ssh/config 导入",
      title: "从 ~/.ssh/config 导入",
      loading: "正在读取 ~/.ssh/config…",
      emptyTitle: "没有可导入的主机",
      emptySub: "在 ~/.ssh/config 里没有找到可用的 Host 条目。",
      importableTitle: "可导入的主机（{count}）",
      skippedTitle: "已跳过（{count}）",
      overwriteBadge: "更新已有",
      overwriteTitle: "已保存的主机里已经有这个别名，导入会就地更新它",
      proxyBadge: "{label} · 不能直接连接",
      importing: "导入中…",
      confirm: "导入选中的主机（{count}）",
    },
  },
  settings: {
    title: "设置",
    tabs: {
      general: "通用",
      relay: "Relay",
      logging: "日志",
      updates: "更新",
      plugins: "插件",
      shortcuts: "快捷键",
      devices: '已登录设备',
      receivedFiles: '接收文件',
      mobileProfiles: '同步的配置',
      mobileHosts: 'SSH 主机',
    },
    discardRelayTitle: "放弃未保存的 relay 更改？",
    discardRelayBody: "你对 relay URL、token、权限或 insecure mode 的编辑尚未保存。",
    general: {
      languageLabel: "语言",
      languageHint: "选择跟随系统时，会使用设备语言。",
      terminalTheme: "终端主题",
      terminalThemeHint: "立即应用到所有终端面板，并保存为本地桌面偏好。",
      terminalFont: "终端字体",
      terminalFontHint: "所选字体会排在内置回退链之前；中日韩字符仍由内置字体族渲染。",
      terminalFontSize: "字号",
      terminalLineHeight: "行高",
      terminalCursorStyle: "光标样式",
      terminalCursorStyleBlock: "方块",
      terminalCursorStyleUnderline: "下划线",
      terminalCursorStyleBar: "竖线",
      terminalCursorBlink: "光标闪烁",
      terminalScrollback: "回滚行数",
      terminalScrollbackHint: "当前设置下每个 pane 约占 {mb} MB，开几个就是几份。",
      notificationsBell: "终端响铃时显示系统通知",
      notificationsHint: "仅在 AT Term 窗口未聚焦时触发。",
      shellIntegration: "启用 shell 集成",
      shellIntegrationHint: "在会话启动时向 zsh / bash / fish / pwsh 注入 OSC 133 hooks，用于检测命令何时完成。仅影响新会话。",
      recoveryEnabled: "启动时检测并提示恢复上次会话",
      recoveryEnabledHint: "在启动时询问是否恢复上次的标签页和分屏。",
      defaultShell: "默认 shell",
      auto: "自动",
      customPath: "自定义路径...",
      defaultShellHint: "自动为新本地会话选择平台默认 shell。也可以选择检测到的 shell，或保存自定义 shell 路径，例如 Git Bash、PowerShell 7 或其他终端程序。",
      customShellPath: "自定义 shell 路径",
      customShellRequired: "必须填写自定义 shell 路径",
      saveCustom: "保存自定义项",
      webglRenderer: "使用 WebGL 终端渲染器",
      webglHint: "GPU 光栅化渲染可避免浅色主题在密集 TUI 输出中出现单元格残影。Linux 默认关闭，因为 NVIDIA 专有驱动 + X11 + WebKitGTK 会让光标/最后一个单元格晚一帧绘制，表现为可见输入延迟。仅影响新终端会话。",
      commandNotifyThreshold: "命令完成通知阈值（秒）",
      commandNotifyHint: "短于此时长的命令不会产生通知。需要启用 shell 集成。",
      pushNotifications: {
        title: "推送通知",
        enable: "启用推送通知",
        test: "发送测试通知",
        errors: {
          denied: "浏览器拒绝了通知权限。",
          disabled: "服务器已禁用推送通知。",
          keyFailed: "无法从 relay 获取推送密钥。",
          subscribeFailed: "浏览器订阅失败。",
          subscribeRejected: "relay 拒绝了此订阅。",
          generic: "推送通知请求失败。",
        },
      },
      themes: {
        classicLabel: "AT Term 经典",
        classicDescription: "当前的黑色终端外观。",
        nordLabel: "Nord / Arctic",
        nordDescription: "冷色低对比度深色调色板。",
        solarizedDarkLabel: "Solarized Dark",
        solarizedDarkDescription: "经典暖色低对比度深色调色板。",
        daylightLabel: "Daylight",
        daylightDescription: "适合明亮环境的浅色调色板。",
      },
    },
    configio: {
      title: "导入与导出",
      export: {
        title: "导出",
        hint: "将偏好设置、SSH 主机和会话配置导出为 JSON 文件，可拷贝到另一台设备上使用。文件中不会包含你的 SSH 密码或私钥。",
        includeEnvLabel: "包含未设置同步的会话配置环境变量（可能含有 API token 等敏感信息）",
        button: "导出…",
        exporting: "导出中…",
        success: "已保存到 {path}",
      },
      import: {
        title: "导入",
        hint: "选择之前导出的 JSON 文件。确认前不会应用任何更改。导入的 SSH 主机不会带来凭据：新增的主机需要之后在这里补充密码或私钥；如果某个已有主机的地址发生了变化，其已保存的凭据会为安全起见被清除，同样需要重新填写。",
        pickFile: "选择文件…",
        fileLabel: "文件：{name}",
        previewing: "正在读取文件…",
        fileReadError: "无法读取文件",
        previewError: "无法读取该文件：{error}",
        previewTitle: "预览 — 尚未应用",
        changesAdd: "新增",
        changesReplace: "替换",
        changesUnchanged: "无变化",
        noChanges: "没有需要变更的内容。",
        skippedTitle: "已跳过",
        apply: "应用导入",
        applying: "应用中…",
        applyError: "导入失败：{error}",
        appliedTitle: "导入已应用",
        appliedSummary: "已应用 {count} 项变更。",
      },
    },
    relay: {
      uplink: "Uplink",
      off: "关",
      on: "开",
      connectedAs: "已连接为 {identity}",
      notConfigured: "未配置",
      paused: "已暂停（保留配置）",
      connecting: "正在连接…",
      relayInvalid: "relay 地址无效",
      versionProbeFailed: "无法连接到 relay：{reason}",
      loginFailedInline: "登录失败：{reason}",
      credentialsRequired: "请填邮箱和密码",
      hint: "配置远端 atterm-relay 后，此机器上的会话可从其他设备附加。无人附加时，不会有字节离开此机器。",
      relayUrl: "relay url",
      preferDirectConnection: "优先使用直连",
      preferDirectConnectionHint: "Relay 完成连接和历史恢复后，尝试建立端到端加密的 P2P 终端通道；直连失败时继续使用 Relay。该设置仅对当前设备生效；桌面端开启后，也会接受其他设备发起的直连。",
      scheme: "Relay 协议",
      email: "邮箱",
      password: "密码",
      passwordShow: "显示密码",
      passwordHide: "隐藏密码",
      modeLogin: "登录",
      modeRegister: "注册",
      noAccountPrompt: "没有账号？",
      haveAccountPrompt: "已有账号？",
      claimToken: "领取码（可选）",
      claimTokenPlaceholder: "claim_…",
      claimTokenHint: "粘贴 `atterm-relay` bootstrap 输出的领取码以注册为管理员账户；留空则注册为普通用户。",
      insecureMode: "信任自签证书",
      allowCleartext: "允许本机外的明文 HTTP",
      insecureWarning: "跳过该 relay 的 TLS 证书校验，使自签证书的 relay 可被连接。连接仍然是加密的，但能够中间人拦截的攻击者可冒充该 relay。仅对你自己掌控的 relay 启用（例如 atterm-relay 的自签 quick-start）；对外公开服务请使用正式证书。",
      cleartextWarning: "HTTP 没有 TLS 传输加密，仅建议用于 loopback 本地开发 Relay；非本机地址还必须显式勾选上方明文连接选项。",
      disableE2EE: "禁用端到端加密（测试 / 调试用）",
      disableE2EEWarning: "会话内容（终端输出、标题 / cwd / 当前命令、命令完成通知）将不经端到端封装直接发送给 relay，relay 服务器本身即可读取。传输层 TLS（若启用）仍然生效。账号密钥仍保留，仅暂停本机出站封装。开启期间标题栏会持续显示 ⚠ Plaintext 提醒。",
      saving: "保存中...",
      saveConnect: "保存并连接",
      clearTitle: '清理 relay 信息',
      clearHint: '这将清除本机保存的 relay 地址、账号和登录信息。已经打开的本地终端不受影响。',
      clearAction: '清理',
      clearing: '清理中…',
      clearConfirm: '确定要清除本机保存的 relay 信息吗？',
      clearFailed: '清理失败：{reason}',
      pairing: {
        title: "移动端配对",
        hint: "生成一次性二维码，让手机无需复制 token 即可接入此 relay。",
        generate: "生成二维码",
        generating: "生成中...",
        regenerate: "重新生成",
        expiresIn: "剩余",
        expired: "已过期 - 请重新生成",
        wrappedBadge: "含 E2EE 密钥",
        unwrappedWarning: "当前 E2EE 密钥未解锁 — 手机端扫码后将看不到会话详情。请先在上方 Relay 设置里用邮箱/密码登录以解锁。",
      },
    },
    logging: {
      title: "桌面日志",
      noContent: "暂时没有可用的日志文件内容。",
      truncated: "正在显示日志文件的最近部分。",
      writeLogs: "将日志写入文件",
      currentFile: "当前日志文件",
      changeLocation: "更改位置",
      resetDefault: "重置默认值",
      viewLogs: "查看日志",
      liveTail: "实时尾部（每 3 秒刷新）",
      levelFilter: "显示",
      tagFilter: "子系统",
      tagFilterAll: "全部",
      writeLevel: "日志级别",
      writeLevelHint:
        "写入文件的最低级别。DEBUG 会记录逐帧、逐按键的细节——复现问题时有用，平时很吵。",
      ptyInputDebug: "记录终端输入字节",
      ptyInputDebugHint: "排查输入丢失 / 卡死。不受上面的日志级别限制，勾选即记录。",
      tailPaused: "已暂停",
      tailPausedHint: "向上滚动时已暂停跟随 —— 点此或滚动到底部恢复",
    },
    updates: {
      devDisabled: "开发构建 - 自动更新已禁用",
      checking: "检查中...",
      readyToInstall: "{version} 已下载 - 可安装",
      downloadingStatus: "正在下载 {version} ({pct}%)",
      available: "{version} 可用",
      upToDate: "已是最新 - 上次检查 {ago}",
      notChecked: "尚未检查",
      justNow: "刚刚",
      minutesAgo: "{count} 分钟前",
      hoursAgo: "{count} 小时前",
      daysAgo: "{count} 天前",
      currentVersion: "当前版本",
      status: "状态",
      downloadPath: "下载路径",
      autoCheck: "自动检查更新",
      ghProxy: "GitHub 下载代理",
      proxyHint: "可选。只有 GitHub Release 文件下载会使用此代理；更新检查与签名验证保持不变。",
      releaseNotes: "发布说明",
      checkNow: "立即检查",
      versionLine: "{minor} 线",
      downloadVersion: "下载 {version}",
      cancelDownload: '取消 ({pct}%)',
      cancelling: '取消中…',
      redownload: '重新下载',
      redownloadPrompt: '本机已下载过 {version} 的安装包。是否重新下载？',
      forceInstallRestart: "强制安装并重启",
      installTitle: "安装 AT Term {version}",
      installIntro: "AT Term 将退出并以新版本重新启动。",
      installThisWill: " 这将：",
      forceInstall: "强制安装",
      installRestart: "安装并重启",
    },
    plugins: {
      hint: "插件按需加载。禁用的插件不会影响启动时间或内存。",
      panelDragHint: "可在面板中拖拽调整面板宽度和内部比例。显示选项在文件树的右键菜单中。",
      noneRegistered: "未注册插件。",
    },
    shortcuts: {
      keyboardShortcuts: "键盘快捷键",
      pane: "面板",
      tab: "标签页",
      sidebar: "侧栏",
      pressKey: "按一个键...",
      resetTo: "重置为 {binding}",
      conflictsWith: "冲突：{labels}",
      resetAll: "全部重置为默认值",
      splitPaneVertically: "垂直分割面板",
      splitPaneHorizontally: "水平分割面板",
      closePane: "关闭面板",
      focusPaneLeft: "聚焦左侧面板",
      focusPaneRight: "聚焦右侧面板",
      focusPaneUp: "聚焦上方面板",
      focusPaneDown: "聚焦下方面板",
      newTab: "新建标签页",
      previousTab: "上一个标签页",
      nextTab: "下一个标签页",
      focusSidebarSearch: "聚焦侧栏搜索",
      terminalSearch: "搜索终端",
    },
    diagnostics: {
      tab: "诊断",
      section: "诊断快照",
      hint: "桌面端运行时快照，API token 和请求头已脱敏。适合反馈问题时附上。",
      copy: "复制",
      export: "导出…",
      refresh: "刷新",
      copied: "已复制到剪贴板",
      copyFailed: "复制失败",
      exported: "已保存到 {path}",
    },
    feishu: {
      title: "飞书集成",
      disabled: "飞书集成尚未启用。",
      relay_disabled: "服务端（relay）已关闭飞书集成。",
      load_error: "无法获取飞书状态：{error}",
      retry_status: "重试",
      mode: {
        label: '模式',
        options: {
          auto:  '自动（跟随 Relay 登录态）',
          local: '强制 Local（本机 LongConn）',
          relay: '强制 Relay（依赖 Relay 转发）',
        },
        effective: {
          label: '生效模式',
          fallbackWarn: '已请求 relay 但未登录 / UPLINK 关闭',
        },
      },
      bound: "已绑定到 open_id：{open_id}",
      save: "保存凭据",
      begin_pair: "开始绑定",
      pair_hint: "请在 15 分钟内向机器人发送私聊：/bind {code}",
      delete: "解绑",
      configured: "凭据已保存",
      app_id_label: "App ID",
      callback_label: "事件回调地址",
      callback_hint_relay: "在飞书「事件订阅」选择『将事件发送至开发者服务器』，并填入上面的地址。",
      longconn_hint_local: "本地长连接模式：在飞书「事件订阅」选择『使用长连接接收事件』，无需填回调地址。",
      copy: "复制",
      copied: "已复制",
      reconfigure: "重新配置凭据",
      aiOnlyNotifications: "仅 AI 会话通知",
      remoteTerminal: {
        enable: "启用飞书远程接管",
        autoAttachLabel: "自动接管会话",
        autoAttach: {
          ai: "仅 AI 会话",
          all: "所有会话",
          none: "不自动接管",
        },
      },
      hook_install: {
        enable: "自动安装 Claude Code Hook",
        retry: "重试",
        healthy: "Hook 已安装且健康",
        needs_attention: "Hook 需要修复",
        disabled: "Hook 自动安装已关闭",
      },
      test_send: {
        title: "测试发送",
        hint: "向已绑定的飞书账号发送一张测试卡片，验证通知链路是否打通。",
        command_success: "命令完成（成功）",
        command_failure: "命令完成（失败）",
        command_sealed: "命令完成（仅本机可见）",
        waiting_input: "等待输入",
        sending: "发送中…",
        sent: "已发送，请在飞书查看",
        failed: "发送失败：{error}",
      },
    },
    templates: {
      tab: "快捷模板",
      intro: "每个模板被点击时会把它的文本发到当前会话。",
      add: "新增模板",
      label: "标签",
      text: "发送文本",
      hotkey: "快捷键（如 Alt+1）",
      showBar: "在终端上方显示快捷模板栏",
      save: "保存",
      edit: "编辑",
      delete: "删除",
      reset: "恢复默认",
      resetConfirm: "用默认列表覆盖当前模板？",
    },
    profiles: {
      tab: "会话配置档",
      intro: "配置档（profile）是新开 tab / split 时用的一组命名启动配置：shell、启动目录、启动命令、环境变量。",
      precedenceHint: "新建会话的优先级：显式选的配置档 > 下面设置的默认配置档 > 通用设置里的默认 shell + 主目录。配置档不会取代默认 shell——没有配置档生效时，用的还是它。",
      envPrivacyIntro: "环境变量默认只留在这台机器，不会同步。想同步某个配置档的环境变量，在它上面单独打开「同步这些环境变量」。",
      empty: "还没有配置档。",
      add: "新增配置档",
      edit: "编辑",
      delete: "删除",
      save: "保存",
      name: "名称",
      shellPlaceholder: "shell 路径（留空则用默认 shell）",
      cwdPlaceholder: "启动目录（留空则用主目录）",
      shellUnset: "默认 shell",
      cwdUnset: "主目录",
      startupCmdPlaceholder: "启动命令（可选）",
      startupCmdHint: "在 shell 出现第一个提示符后执行一次，走的是 AI 会话恢复同一条路径——不是按原始按键发送。",
      envPlaceholder: "KEY=值，一行一个",
      envHint: "TERM 始终受保护：配置档无法覆盖它，以保证终端渲染正常。",
      syncEnv: "把这些环境变量同步到我的其他设备",
      syncEnvOffHint: "关闭（默认）：这些环境变量只留在这台机器，同步前会被剥离；名称、shell、目录、启动命令等其余字段仍会同步。",
      syncEnvOnHint: "开启：这些环境变量会加密同步到你其他已登录的设备，端到端封装，relay 读不到明文。",
      setDefault: "设为默认",
      clearDefault: "取消默认",
      defaultBadge: "默认",
      nameRequired: "请填写名称。",
    },
    mobileSync: {
      locked: "正在等待账户密钥——请确认已登录。",
      noData: "还没有任何桌面同步过数据。",
      error: "无法读取同步的数据：{error}",
    },
    mobileProfiles: {
      intro: "从桌面同步的配置，仅供查看。请在运行它们的机器上编辑。",
      empty: "桌面上还没有会话配置。",
      defaultBadge: "默认",
      shell: "Shell",
      cwd: "工作目录",
      startupCmd: "启动命令",
      env: "环境变量",
      envNone: "无",
      envNotSynced: "未从该机器同步",
      hostLabel: "在此桌面打开",
      noHosts: "没有找到可用的桌面。请先在这个账号下的某台桌面上打开一个会话，这样手机才能发现它。",
      openButton: "打开",
      opening: "正在打开…",
      openErrors: {
        unknown_profile: "该配置在桌面上已不存在。",
        permission_denied: "该桌面以只读方式发布，无法从手机新建会话。",
        request_in_flight: "已有一个创建请求正在处理，请等它完成后再试。",
        duplicate_request_id: "该请求已发送过，请等它完成。",
        unknown_host_id: "暂时无法连接到该桌面。",
        invalid_request: "请求发送失败，请重试。",
        upstream_unavailable: "无法连接到该桌面，请重试。",
        session_create_busy: "该桌面当前繁忙，请稍后重试。",
        timeout: "等待桌面响应超时，系统不会自动重试——请先检查桌面状态再重试。",
        relay_not_configured: "请先在此手机上登录 relay。",
        generic: "无法创建会话：{error}",
      },
    },
    mobileHosts: {
      intro: "从桌面同步的 SSH 主机，仅供查看。此设备无法连接它们。",
      empty: "你的桌面还没有配置 SSH 主机。",
      user: "用户",
      port: "端口",
      tags: "标签",
      note: "备注",
      jumpChain: "需要通过跳板链才能到达此主机",
      proxyCommand: "使用 ProxyCommand——不能直接连接",
      key: "密钥",
    },
    devices: {
      hint: '此账号已登录的浏览器 / 桌面 / 移动设备。',
      refresh: '刷新',
      signOutOthers: '登出其他设备',
      signingOut: '登出中…',
      signOutOthersConfirm: '确定要登出除本机外的所有设备吗？',
      revoke: '撤销',
      revoking: '撤销中…',
      revokeConfirm: '确定要撤销此设备 ({ua}) 吗？',
      currentTag: '当前',
      loginLine: '登录于 {time} · {ip}',
      timeFormat: '{y}年{m}月{d}日 {hh}:{mm}',
      empty: '尚未有登录设备。',
      unknownUA: '未知设备',
      notAuthenticated: '请先在 "Relay" 页登录 relay 账号。',
    },
    account: {
      title: '账号',
      email: '邮箱',
      logout: {
        button: '退出登录',
        submitting: '退出中...',
      },
      traffic: {
        title: '流量统计',
        period: '最近 {days} 天（UTC）',
        range: '时间范围',
        dayUnit: '天',
        refresh: '刷新流量统计',
        relay: 'Relay',
        p2p: 'P2P 直连',
        kpiTotal: '总流量',
        kpiOutbound: '出站流量',
        kpiInbound: '入站流量',
        kpiFrames: '帧数',
        trendNew: '上期为 0',
        trendFlat: '持平',
        p2pTraffic: '直连流量',
        offloadShare: '卸载占比',
        offloadHint: '无需 Relay 转发的流量占比',
        successRate: '直连成功率',
        fallbacks: '回退次数',
        fallbackHint: '重新回到 Relay 的连接次数',
        inOut: '入站 {incoming} · 出站 {outgoing}',
        sentReceived: '发送 {sent} · 接收 {received}',
        connectionCounts: '尝试 {attempts} · 直连 {successes} · 回退 {fallbacks}',
        dailyTrend: '每日 Relay 与 P2P 流量',
        coreAnalysis: '核心分析',
        timelineTitle: 'Relay 流量时序',
        view: '视图',
        groupTimeline: '流量类别',
        frameTimeline: '帧类型',
        directionTrend: '方向走势',
        colDirection: '方向',
        metric: '指标',
        bytes: '字节',
        frames: '帧数',
        direction: {
          all: '全部',
          out: '出站',
          in: '入站',
        },
        dirOut: '出站',
        dirIn: '入站',
        categorySummary: '类别摘要',
        totalComposition: 'Relay 流量构成',
        outIn: '出 {out} · 入 {incoming}',
        frameBreakdown: '帧类型构成',
        topFrames: '按流量排序的主要帧类型',
        colFrameType: '帧类型',
        colBytes: '字节',
        colFrames: '帧数',
        directQuality: '直连质量',
        aggregateOnly: '仅展示聚合统计',
        categories: {
          terminal: '终端转发',
          state: '状态同步',
          config: '配置同步',
          fs: '文件浏览',
          preview: '远程预览',
          other: '其他',
        },
        empty: '该时段尚无流量记录。',
        loadFailed: '流量统计加载失败。',
        retry: '重试',
      },
      changePassword: {
        title: '修改密码',
        oldPassword: '当前密码',
        newPassword: '新密码',
        confirmPassword: '确认新密码',
        hintLength: '新密码至少需要 8 位。',
        hintMismatch: '两次输入的密码不一致。',
        submit: '更新密码',
        submitting: '更新中…',
        successToast: '密码已更新',
      },
      dangerZone: {
        title: '危险区',
        warning: '注销账号是永久性操作，无法撤销。',
        toggle: '注销账号',
        currentPassword: '当前密码',
        typeEmailToConfirm: '输入你的邮箱以确认',
        deleteButton: '注销账号',
        deleting: '注销中…',
        confirmPrompt: '此操作不可恢复。',
      },
      errors: {
        wrongPassword: '旧密码不正确。',
        sessionExpired: '会话已过期，请重新登录。',
        rateLimited: '请稍后重试。',
        network: '网络错误。',
        generic: '操作失败，请重试。',
      },
    },
  },
  mobile: {
    relayUrl: "Relay URL",
    email: "邮箱",
    password: "密码",
    passwordShow: "显示密码",
    passwordHide: "隐藏密码",
    emailRequired: "请输入邮箱",
    passwordRequired: "请输入密码",
    invalidCredentials: "邮箱或密码错误",
    rateLimited: "操作过于频繁，请稍后再试",
    loginButton: "登录",
    allowInsecure: "允许 insecure HTTP/WS（非 loopback）",
    originRejected: "Relay 拒绝了此 origin。请启动 relay 时让 ATTERM_ORIGINS 包含 capacitor://localhost。",
    cannotReachRelay: "无法连接 relay：{message}",
    relayUrlRequired: "必须填写 relay URL",
    relayUrlMalformed: "relay URL 无效或格式错误",
    relayUrlProtocol: "relay URL 必须以 http:// 或 https:// 开头",
    relayUrlNoPath: "relay URL 不能包含路径、查询或片段",
    relayUrlInsecure: "若要将 http:// 用于非 loopback 主机，请启用 \"允许 insecure HTTP/WS\"",
    taskSections: {
      needs_attention: "需要处理",
      running: "运行中",
      completed: "已完成",
      failed: "失败",
      disconnected: "已断开",
    },
    taskStates: {
      idle: "空闲",
      running: "运行中",
      waiting_input: "等待输入",
      completed: "已完成",
      failed: "失败",
      disconnected: "已断开",
      closed: "已关闭",
    },
    taskTypes: {
      ai: 'AI',
      test: '测试',
      build: '构建',
      deploy: '部署',
    },
    started: "开始于",
    lastOutput: "最近输出",
    openBadge: "已打开",
    back: "返回",
    pasteClipboard: "粘贴",
    pasteImage: "图片",
    pasteConfirm: "确认粘贴",
    pastePreview: "剪贴板文本",
    protectMode: {
      banner: "防误触模式 · 打开下方控制模式可输入",
    },
    settings: {
      title: "设置",
      back: "返回",
      language: "语言",
      templates: "快捷模板",
      auxKeys: "快捷按键",
      logout: "退出登录",
      add: "新增",
      edit: "编辑",
      delete: "删除",
      reset: "恢复默认",
      resetConfirm: "用默认列表覆盖当前设置？",
      label: "标签",
      text: "发送文本",
      seq: "发送字节（\\r \\n \\t \\e \\xNN ^X）",
      showBar: "在终端上方显示快捷模板栏",
      save: "保存",
    },
    image: {
      prompt: "添加图片",
      fromLibrary: "从相册选择",
      takePhoto: "拍照",
    },
    pairing: {
      connecting: "配对中…",
      failed: "配对失败",
      back: "重试",
      scan: "扫描二维码",
      cameraDenied: "需要相机权限才能扫描二维码",
      noQrDetected: "未识别到二维码，请重试",
      scanNotAvailable: "此版本暂不支持扫码功能，请在下方手动填入 relay 地址",
      errInvalidUrl: "二维码内容不是合法的配对链接",
      errInvalidScheme: "二维码地址不是 https，无法配对（如需 http 请回到设置打开 insecure 开关）",
      errPlatformUnsupported: "当前平台不支持二维码配对",
      errCannotReachRelay: "无法连接 relay，请检查二维码地址或网络后重试",
      errTimeout: "配对请求超时（15s），请确认 relay 可达后重试",
      errPairInvalid: "二维码配对令牌无效或已过期，请重新生成",
      errCancelled: "已取消配对",
      errGeneric: "配对失败：{message}",
      stepParsing: "解析二维码…",
      stepRequesting: "请求 relay 服务器…",
      stepSaving: "保存配置…",
    },
    insecure: {
      warning: {
        title: "不安全的 relay (HTTP)",
        body: "API token 和终端输出会以明文方式传输。生产环境请使用 HTTPS。",
        dismiss: "知道了",
      },
    },
    selection: {
      copy: "复制",
      send: "发送",
      cancel: "取消",
      copied: "已复制到剪贴板",
      copyFailed: "复制失败",
    },
  },
  tasks: {
    bulk: {
      selectedCount: "已选 {count}",
      mergeTab: "合并 tab",
      mergeTooMany: "最多合并 4 个",
      close: "关闭 ({count})",
      closeHint: "关闭 {count} 个已打开的 pane",
      closeNoneOpen: "选中集合中没有本窗口已打开的会话",
      cancel: "取消",
    },
    details: {
      title: "会话详细信息",
      sessionId: "Session ID",
      type: "类型",
      host: "主机",
      cwd: "工作目录",
      command: "命令",
      state: "状态",
      startedAt: "开始于",
      commandStartedAt: "命令开始",
      commandEndedAt: "命令结束",
      commandDuration: "命令耗时",
      commandExitCode: "退出码",
      unread: "未读",
      pinned: "已置顶",
      paneLocation: "所在位置",
      paneAt: "Tab {tab} · Pane {pane}",
      paneNone: "本窗口未打开",
    },
    rowMenu: {
      details: "详细信息",
      merge: "合并 tab ({n})",
      closeSelected: "关闭已打开 ({n})",
    },
    sidebar: {
      title: "会话",
      collapse: "折叠侧栏 (Cmd/Ctrl+B)",
      expand: "展开侧栏 (Cmd/Ctrl+B)",
      searchPlaceholder: "搜索",
      openDrawer: "打开侧栏",
      closeDrawer: "关闭侧栏",
    },
    preset: {
      iconOnly: {
        name: "仅图标",
        description: "用颜色 + 图标传达状态。最紧凑。",
      },
      iconLabel: {
        name: "图标 + 文案",
        description: "图标旁加一段「运行中 / 等待输入 / 已完成」文字。更直观。",
      },
    },
    pinned: {
      title: "置顶",
      menuPin: "置顶",
      menuUnpin: "取消置顶",
    },
    search: {
      empty: '搜索 "{q}" 无匹配会话',
    },
    markAllRead: "全部标已读",
    completedFold: "已完成",
    unreadBadge: "{count} 条未读",
    settings: {
      section: "会话栏",
      preset: "显示样式",
      expandByDefault: "默认展开会话栏",
      groupBy: "分组方式",
      groupByHost: "按主机",
      groupByState: "按状态",
    },
  },
  plugins: {
    fileExplorer: {
      title: "文件浏览器",
      description: "侧边面板，包含文件树和只读语法高亮预览。跟随当前面板的 cwd。",
      noActivePaneShort: "(无活动面板)",
      noActivePane: "无活动面板。",
      selectFile: "选择一个文件。",
      pinCurrentCwd: "固定当前 cwd",
      unpinFollow: "取消固定（跟随活动面板）",
      showHidden: "显示隐藏文件",
      showLineNumbers: "在编辑器中显示行号",
      searchFiles: "搜索文件名",
      noSearchResults: "没有匹配的文件。",
      searchTruncated: "仅显示前 200 个匹配项。",
      fileChanged: "磁盘上的文件已更改",
      reload: "重新加载",
      tooLarge: "文件过大，无法预览。请在外部打开。",
      binary: "二进制文件 - 无法预览。",
      errorPrefix: "错误：{message}",
      unexpectedContentType: "文件内容类型异常",
      unknownError: "预览失败，原因未知",
      openInSystem: "用系统应用打开",
      unsupportedPreview: "此文件类型暂不支持内嵌预览。",
      showAsCode: "看源码",
      showAsRender: "看渲染",
      save: "保存",
      dontSave: "不保存",
      cancel: "取消",
      dirty: "有未保存的更改",
      staleModTime: "文件在你打开后已被外部修改。",
      overwrite: "覆盖",
      reloadDiscard: "重新加载（放弃更改）",
      confirmCloseTitle: "是否保存对 {name} 的更改？",
      newFile: "新建文件",
      newFolder: "新建文件夹",
      rename: "重命名",
      delete: "删除",
      copyPath: "复制路径",
      sendToTerminal: "终端引用",
      pathCopied: "路径已复制",
      moveToTrash: "移到回收站",
      confirmTrash: "把 {name} 移到回收站？",
      confirmHardDelete: "永久删除 {name}？此操作无法撤销。",
      saveFailed: "保存失败：{message}",
      // --- SSH 主机数据源（roadmap 第 28 项）---
      sourceLabel: "文件来源",
      sourceActivePane: "当前面板",
      listingTruncated: "{path}：共 {total} 项，主机只返回了 {shown} 项，其余未列出。",
      sshUpload: "上传文件到 {dir}",
      sshUploadDone: "已把 {name} 上传到 {dir}",
      sshUploadExists: "远端已存在 {path}，没有写入任何内容——那边没有回收站，覆盖无法撤销。请先重命名或删除它。",
      sshNoOpenExternal: "SSH 主机上的文件无法交给本机的系统应用打开。",
      sshNoInlineAsset: "SSH 主机上的这类文件无法内嵌预览。",
      sshNoRecursiveDelete: "{path} 是 SSH 主机上的目录。删除它会连同里面的一切一起永久删除——那边没有回收站，也无法撤销——所以面板不提供这个操作。请在该主机的终端里删除。",
      sshBridgeDisposed: "该 SSH 文件来源已关闭。",
      treeLoadFailed: "无法列出 {path}：{message}",
      retry: "重试",
    },
    deskWidget: {
      title: "桌面挂件",
      description:
        "一个置顶的小窗口，显示哪些会话在跑、失败了、或者在等你输入——AI agent 和普通 shell 一视同仁。点会话行可直接跳到对应标签页。",
      aiOnly: "仅显示 AI 会话",
      aiOnlyHint:
        "只显示跑着 claude / codex / aider 的会话。默认关闭：会话状态来自 OSC 133，任何命令都适用。",
    },
    translate: {
      title: "翻译",
      description: "通过兼容 OpenAI 的 API 翻译选中文本，并在浮动面板中显示结果。",
      selection: "翻译选中文本",
      notConfigured: "翻译插件尚未配置",
      closePanel: "关闭翻译面板",
      source: "原文",
      detected: "检测到 {lang}",
      target: "目标语言",
      translating: "翻译中...",
      noResult: "还没有结果。",
      recent: "最近 ({count})",
      baseUrl: "Base URL",
      apiKey: "API Key",
      model: "模型",
      defaultTargetLanguage: "默认目标语言",
      keyStoredPlaintext: "API key 以明文存储在 {path}。",
      extraParams: "额外参数",
      extraParamsHint: "合并进每次请求 body 的 JSON 对象（例如 stream、top_p、max_tokens），会覆盖默认值。",
      extraParamsInvalid: "不是合法的 JSON 对象。",
      invalidExtraParams: "额外参数无效：{detail}",
      simplifiedChinese: "中文（简体）",
      targetEnglish: "英语",
      targetJapanese: "日语",
      targetKorean: "韩语",
      targetGerman: "德语",
      targetFrench: "法语",
      targetSpanish: "西班牙语",
      userCancelled: "用户已取消",
      requestTimedOut: "请求在 {seconds}s 后超时",
      authFailed: "认证失败 ({status})",
      rateLimited: "请求过于频繁 (429)",
      serverError: "服务器错误 ({status})",
      providerNonJson: "provider 返回的外层响应不是 JSON",
      missingTranslated: "缺少 translated 字段",
      responseFormatUnsupported: "不支持 response_format",
    },
  },
  connHealth: {
    pillLabelConnecting: "连接中…",
    pillLabelReconnecting: "重连中…",
    pillLabelOff: "已关闭",
    drawerTitle: "连接质量",
    drawerRttNow: "RTT(当前)",
    drawerRttP50P95: "p50 / p95(最近 5 分钟)",
    drawerBytesIn: "↓ 接收",
    drawerBytesOut: "↑ 发送",
    drawerState: "状态",
    drawerReconnectsLastHour: "最近 1 小时重连",
    drawerReconnectsTime: "时间",
    drawerReconnectsReason: "原因",
    drawerReconnectsDowntime: "断开时长",
    drawerSeqGaps: "观察到的序号跳跃:",
  },
  startupUpdate: {
    title: "有可用更新",
    currentToLatest: "{current} → {latest}",
    releaseNotes: "更新说明",
    statusAvailable: "{version} 可用",
    statusDownloading: "正在下载 {version}（{pct}%）",
    statusReady: "{version} 已下载 — 可安装",
    downloadInstall: "下载并安装",
    cancel: "取消（{pct}%）",
    cancelling: "正在取消…",
    installRestart: "安装并重启",
    later: "稍后",
  },
  recovery: {
    dialog: {
      title: "恢复上次的会话？",
      subtitleClean: "上次正常退出 · {minutes} 分钟前",
      subtitleUnclean: "上次异常结束 · {minutes} 分钟前",
      btnRestoreAll: "恢复全部 {count} 个",
      btnRestoreSelected: "恢复选中的 {count} 个",
      btnDiscard: "全部丢弃",
      badgeResumable: "可续接",
      badgeFresh: "无 sid（不自动续接）",
      badgeShell: "（无 AI）",
      badgeUnclassified: "未识别为 AI",
    },
    pane: {
      failed: "（已失败）",
    },
  },
  snippets: {
    runOnHosts: "在多台主机上运行…",
    selectHosts: "选择主机",
    pending: "等待中",
    running: "运行中…",
    ok: "成功",
    failed: "失败",
    error: "出错",
    exitCode: "退出码 {code}",
    truncated: "输出已截断",
    copyAll: "复制全部",
    cancel: "取消",
    noHostsSelected: "请至少选择一台主机",
  },
  sync: {
    stateIdle: "已是最新",
    stateSyncing: "同步中…",
    stateOffline: "未同步",
    stateError: "同步失败",
    syncNow: "立即同步",
    lastSyncedNever: "从未同步",
    lastSyncedAt: "上次同步 {time}",
    pendingKeys: "还有 {count} 项改动待同步",
    pulledAdopted: "已从其他设备更新：{keys}",
    pulledConflict: "保留了本机版本（其他设备也改过）：{keys}",
    keys: {
      locale_preference: "语言",
      quick_templates: "快捷模板",
      notifications_enabled: "通知",
      ai_notifications_only: "仅 AI 通知",
      command_notify_threshold_seconds: "命令完成通知阈值",
      shell_integration_enabled: "Shell 集成",
      pinned_session_ids: "置顶会话",
      ssh_hosts_encrypted: "SSH 主机",
      terminal_theme: "终端主题",
      terminal_font_head: "终端字体",
      terminal_font_size: "终端字号",
      terminal_line_height: "终端行高",
      terminal_cursor_style: "光标样式",
      terminal_cursor_blink: "光标闪烁",
      terminal_scrollback: "回滚行数",
      default_shell: "默认 shell",
      shortcut_bindings: "键盘快捷键",
      profiles_encrypted: "会话配置",
    },
  },
} as const satisfies Messages;
