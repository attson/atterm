// OPAQUE client backed by the Go bytemare/opaque module compiled to WASM (see
// cmd/opaque-wasm). This replaces @cloudflare/opaque-ts, which is NOT
// cross-client interoperable with the Go desktop + relay (different client AKE
// keypair derivation). The WASM speaks the exact same protocol bytes as the
// desktop, so an account registered here can be logged into from the desktop /
// web and vice-versa.
//
// Duplicate of web/src/shared/lib/opaqueWasm.ts. The two bundles ship to
// different runtimes (Capacitor WKWebView vs browser tab) and keep separate
// node_modules + gitignored wasm artifacts; mechanically identical, so any
// change here MUST be mirrored in the web copy.
//
...
// login_ke, login_response, login_ke3) — so the platform layer passes them
// through without re-encoding. The account-key AEAD wrap (Argon2id + XChaCha20)
// and session-content decrypt stay in JS (noble) in ./opaque.ts; the WASM only
// runs the OPAQUE protocol crypto + message (de)serialization.
//
// Loading is lazy + singleton: the ~1MB (gzip) binary is fetched only when an
// auth action first happens, never at app boot.
...
// `Go` global). Browser/WebView-only; tests mock this module.
...
function init(): Promise<AttermOpaque> {
  ...
  readyPromise = (async () => {
    ...
    try {
      ...
      // Capacitor's WKWebView local scheme handler is one such server, so this
      // branch is the hot path on iOS.
