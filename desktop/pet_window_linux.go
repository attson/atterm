...
// applyPetPlatformOptions configures the companion window for Linux/GTK.
//
// Known gap: GTK's skip-taskbar hint is not exposed by Wails v2, so on some
// desktop environments the pet also appears in the window list. Reaching it
...
// without one the pet renders on an opaque background but stays usable.
...
// applyPetPostStartup runs from OnStartup. Nothing to do on Linux: the
