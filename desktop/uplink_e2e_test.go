...
func TestTwoHostsCrossAttach(t *testing.T) {
	...
	defer cancel()
	u1 := newUplink("ws://"+remoteAddr, remoteTok, proto.RemotePermissionFull, h1, nil, nil, false)
	u1.eventsEmit = func(context.Context, string, ...interface{}) {} // no Wails runtime in tests
	go u1.Run(ctx)
	u2 := newUplink("ws://"+remoteAddr, remoteTok, proto.RemotePermissionFull, h2, nil, nil, false)
...
func TestUplinkE2E(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	u := newUplink("ws://"+remoteAddr, remoteTok, proto.RemotePermissionFull, host, nil, nil, false)
...
func TestUplink_AuthInfo_EmitsUserID(t *testing.T) {
	...
	u := newUplink("ws://"+ln.Addr().String(), "tok", proto.RemotePermissionFull, host, nil, nil, false)
