...
> 多机执行**不是**「把 snippet 打进终端」的批量版，而是另一种操作。往终端里打字要为每台主机开一个 PTY，拿不到退出码，也分不清输出、回显和 shell 噪声——「汇总各主机输出」会变成汇总一堆 transcript。所以这里把 snippet 当**命令**跑：走非 PTY 的 exec 通道，每台主机拿回 `(stdout+stderr, 退出码, 错误)`。设计见 [`2026-08-20-snippets-multi-host-design.md`](./superpowers/specs/2026-08-20-snippets-multi-host-design.md)。
>
> 连接一律经 `dialThroughJumps`，所以跳板链、逐跳 host key 校验、`ProxyCommand` 拒绝全部照第 27 项的规则生效，没有第二套实现（`hostRunsProxyCommand` 仍是两个调用点）。

- [x] 选中 N 台主机执行同一 snippet（并发上限 8，单机 60s 超时，单机输出上限 256 KiB；超限截断并标注，不让运行失败）
- [x] 汇总各主机输出（`ok` / `failed` / `error` 三态分明：`failed` 是**跑了**但退出码非零，退出码与输出都在；`error` 是**没跑成**——不可达、认证被拒、host key 尚未信任、或超时。混为一谈的话，捕获退出码这件事本身就没意义了。连接中途断开前主机已打印的内容也保留，那通常是最有用的诊断信息）
- [x] 增加多机执行测试

**刻意没做**：不落历史、不做调度——「谁在哪台跑过什么」是审计能力，属于第 16 项；不做逐主机变量替换，任何模板语言都会带来自己的一套转义规则；不处理 sudo 密码提示，非 PTY exec 没法应答，会一直挂到 60s 超时（如实记录，不绕）；移动端没有，批量执行和 SSH 主机清单一样只在桌面端。

**host key 不在批量执行中协商**：未信任的主机直接记为 `error`，不弹 TOFU。批量运行时连开 N 个信任弹框，恰好在用户最无法核对指纹的时刻（盯着进度列表）训练出点穿反射，而那正是 TOFU 唯一的价值所在。信任要单独、有意地连一次终端来建立;批量执行只消费信任，从不索取。
...
**导入经既有 setter 写入**，不直接写 configStore——否则导入的值会一直对其他设备不可见，直到别的操作碰巧动到同一个 key。由此带出一条：六个 setter 会做校验（locale、主题、字号、行高、光标样式、scrollback），预览必须跑同一套校验并把不合格项报成 skipped，**且应用失败时继续而不是中止**。中止过一次的后果不是「某个 key 没导入」，而是前面的 key 已经落盘**并推送到所有其他设备**，用户拿到一份既不是文件、也不是原状的配置——而任何跨版本的文件都会触发它，因为主题列表和尺寸范围在不同构建间会变。`ssh_hosts` 是唯一的例外：它的单条 setter `UpdateSSHHost` 会无条件保留已存的 `IdentityFile`/`ProxyJump`/`ProxyCommand`，这对应用内编辑是对的，对导入是错的（文件确实清空某个字段会被悄悄改回本地值），所以导入这一路直接写 configStore、自己标脏，绕过这一个 setter——同时仍然承担「替换的记录改了 Host/Port/User 时清掉该主机在 keyring 里的旧凭据」这份 setter 该做的事（对齐 `DeleteSSHHost` 自己清凭据的写法），不是简单绕过后什么都不管。
...
> **「使用」只能是请求桌面代劳。** `capacitor.ts` 里写着 `newSession omitted — capacitor cannot fork local PTYs`，移动端根本没有创建会话的通路。profile 是「怎么 fork 一个 PTY」的配方，而只有桌面会 fork。所以本项把「使用」实现为：手机选一个 profile 和一台桌面，请那台桌面开会话，然后照既有路径 attach 上去。设计见 [`2026-08-21-mobile-profiles-hosts-design.md`](./superpowers/specs/2026-08-21-mobile-profiles-hosts-design.md)。
>
> **手机只发 profile ID，永不发 profile 本体。** 桌面从自己的配置里按 ID 解析。这是整个设计的安全支点：手机在桌面已有的配方里挑一个，而不能写一个——载荷结构上只有三个字符串字段，连塞 `startup_cmd` 的地方都没有。
>
> **展示这一半的主要风险是跨语言格式漂移**：Go 改一次密封信封，移动端静默失效而 CI 全绿。所以两个读取器各配一份**由 Go 测试生成**的金标向量——Go 改信封就会重写向量，TS 测试随即变红。而且因为 CI 跑 `go test ./desktop/` 时不带 `-update`，默认走的就是「对着已提交向量断言」这条路，CI 本身成了兜底。

- [x] 移动端展示并使用 profile（展示只读；`SyncEnv: false` 的 profile 显示「未从那台机器同步」而不是「无」——Go 在密封前就把 Env 剥掉了，手机确实拿不到，这两件事必须说得不一样。使用走新的 `SESSION_CREATE`/`SESSION_CREATED` 帧对（0x3b/0x3c，不改任何既有帧的 payload，红线 #4），relay 按 `{owner, host_id}` 路由，回复只发给请求方）
- [x] 移动端展示 SSH 主机清单（只读，不含任何凭据。密封块里**装着**密码和私钥，读取器只解出记录、视图类型连字段都没有，并由埋在向量里的 canary 字符串加精确键白名单守着——一次泄漏演练证明单靠字段名黑名单挡不住「把整个对象倾倒进 DOM」）
