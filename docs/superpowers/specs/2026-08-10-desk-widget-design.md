...
- **推送节流是"合并"而不是"丢弃"**：最快 200ms 一次，内容未变则跳过；窗口内到达的更新
  记进 `pending` 并由一次性 timer 补发（`widgetProcess.pushLocked` / `flushPending`）。
  **不要改回直接 return**——`ls` 这类命令在一个窗口内就跑完，"running"→"completed" 的第二条
  会被丢掉，而命令结束后不再有事件来纠正它，挂件就一直显示着已经跑完的命令还在跑。
  `Stop()` 必须取消 pending timer，否则它会写进已经关掉的管道。
...
`WidgetSprite.vue` 是一只 **12×12 像素猫**（inline SVG rect + `shape-rendering="crispEdges"`），
四种表情对应四个 mood。三条像素图专属约束，改动前先看：

- **尺寸必须是 12 的倍数**（当前 36 = 3px/格）。40px 会得到 3.33px/格，部分列比别的宽一像素，
  这在像素图上一眼就看得出来。
- **动画只能整像素平移 + `steps()` 分帧**。小数偏移会重采样整张图，边缘全糊。
- **不能用 `scale()` / `rotate()`**。旋转过的像素猫是一只糊猫——之前几何形象用的
  `scale(1.05)` 和 `rotate(-5deg)` 已经全部替换掉。

`.sprite-wrap` 固定 36×36：动画平移的是 svg 本身，父容器若随内容自适应，右上角的角标会跟着一起动。

不用 emoji 的原因见红线 #13（平台 emoji 字体表现不一，某些 Linux 干脆没有）。
