...
func consumeDirectActiveRoute(peer *directSignalPeer) bool {
	for {
		active := peer.activeRoutes.Load()
		if active == 0 {
			return false
		}
		if peer.activeRoutes.CompareAndSwap(active, active-1) {
			return true
		}
	}
}
