...
type subStore struct {
	mu   sync.Mutex
	byID map[string][]Subscription // userID -> subs
}
...
// Add registers a subscription under the given userID. Same-endpoint
// re-adds overwrite the existing entry (refreshing CreatedAt). Returns nil
// even when the cap is hit (drops silently).
func (s *subStore) Add(userID string, sub Subscription) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	subs := s.byID[userID]
	...
	s.byID[userID] = append(subs, sub)
	return nil
}

// Remove deletes the subscription for the given userID and endpoint.
// Returns true when something was removed.
func (s *subStore) Remove(userID, endpoint string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	subs, ok := s.byID[userID]
	if !ok {
		return false
	}
	for i, existing := range subs {
		if existing.Endpoint == endpoint {
			s.byID[userID] = append(subs[:i], subs[i+1:]...)
			if len(s.byID[userID]) == 0 {
...
// ByUser returns a copy of the slice for the given userID.
func (s *subStore) ByUser(userID string) []Subscription {
	s.mu.Lock()
	defer s.mu.Unlock()
	subs := s.byID[userID]
	if subs == nil {
