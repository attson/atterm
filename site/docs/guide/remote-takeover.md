...
一个置顶的悬浮小窗,盖在浏览器 / 编辑器上面,不用切回 AT Term 就能看到:哪些会话在跑、
哪些失败了、哪些在等你输入。**点会话行**直接把 AT Term 拉到前台并定位到对应 tab/pane。

在 **Settings → 插件 → 桌面挂件** 打开。特性:

- **不只看 AI**。状态来自 OSC 133,`vim` / `make` / 任意脚本一视同仁,不用给每个 agent 装 hook。
  只想看 AI 会话的话有「仅显示 AI 会话」开关。
- **远程会话也在同一个列表里**。relay 上另一台机器的 claude 在等你输入,这里照样看得到。
- **折叠 / 展开**。折叠后只剩一行汇总(仍然写着"1 个等你输入"),鼠标悬停临时展开;
  有会话新变成「等待输入」或「失败」时,折叠态会自动展开 3 秒提醒你。
- **不打扰**。右键可静音 15 分钟 / 1 小时(只停动画,计数照常),或直接隐藏。
  跟随系统「减弱动态效果」设置。
- 位置随手拖,下次启动还在原处。AT Term 退出时挂件一起退出,不会留在桌面上。
